...
3.  安装依赖工具。
    1.  登录[弹性云服务器控制台](https://console.huaweicloud.com/ecm/?region=&locale=zh-cn#/ecs/manager/vmList)，在列表中找到步骤[1](#li1497416395259)中购买的弹性云服务器。
    ...
    3.  执行以下命令，安装运行程序所需的依赖工具。
        ...
        -   拉取Docker-Compose镜像，本文档使用的是1.29.2版本。
    ...
    4.  执行以下命令，检测依赖工具是否安装成功。
        ...
        当出现如[图2](#fig1461153219438)所示提示时，表示安装成功。

        **图 2**  查看Docker及Docker-Compose版本<a name="fig1461153219438"></a>  
...
1.  添加授信主机。
    ...
    3.  当出现如[图3](#fig12579781818)所示的提示时，表示主机添加成功。

        若主机添加失败，请参考[部署-常见问题-主机问题](https://support.huaweicloud.com/deployman_faq/deployman_faq_0000.html)排查。

        **图 3**  添加主机成功<a name="fig12579781818"></a>  
        ![](figures/添加主机成功.png "添加主机成功")

2.  配置部署任务。
    1.  单击页面上方导航“构建&发布  \>  部署“，页面将显示样例项目自动创建的3个部署任务，在列表中找到任务“phoenix-sample-standalone“，如[图4](#fig13474111212120)所示。

        **图 4**  部署<a name="fig13474111212120"></a>  
        ![](figures/部署.png "部署")

    2.  单击图标![](figures/icon-编辑任务.png)进入编辑页面。
    3.  在“部署步骤“页签，编辑部署任务的步骤。
        1.  步骤“选择部署来源“，参照[表3](#table535994471416)设置部署来源，如[图5](#fig7467422928)所示。
            ...
            **图 5**  选择部署来源<a name="fig7467422928"></a>  
...
3.  执行部署任务。
    ...
    任务执行耗时约1\~2分钟，当出现如[图6](#fig327313441538)所示页面时，表示任务执行成功。

    若任务执行失败，请根据报错信息，参考[附录](附录.md)排查。

    **图 6**  部署成功<a name="fig327313441538"></a>  
...
4.  验证部署结果。
    ...
    进入如[图7](#fig14261561312)所示页面，在导航栏中可看到菜单项“门店网络“。

    **图 7**  用户端UI<a name="fig14261561312"></a>  
    ![](figures/用户端UI-2.png "用户端UI-2")

    输入“http://ip:5001“，其中ip为弹性云服务器的IP地址，可以进入如[图8](#fig91205440149)所示页面。

    **图 8**  管理端UI<a name="fig91205440149"></a>  
