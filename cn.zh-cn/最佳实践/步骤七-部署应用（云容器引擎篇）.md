...
2.  配置部署任务。
    1.  进入“凤凰商城“项目，单击页面上方导航“构建&发布  \>  部署“，进入部署服务。
        ...
        找到部署任务“phoenix-cd-cce“，单击此任务对应操作列中的图标![](figures/icon-编辑任务-4.png)，进入“编辑任务“页面。

    2.  单击页面上方导航“构建&发布  \>  部署“，页面将显示样例项目自动创建的部署任务“phoenix-sample-cce“，如[图3](#fig9353102132115)所示。

        **图 3**  部署<a name="fig9353102132115"></a>  
        ![](figures/部署-5.png "部署-5")

    3.  单击图标![](figures/icon-编辑任务.png)进入编辑页面。
    4.  在“部署步骤“页签，编辑部署任务的步骤。
        1.  参照[表2](#table535994471416)配置步骤“选择部署来源“，如[图4](#fig8770121422218)所示。
            ...
            **图 4**  选择部署来源<a name="fig8770121422218"></a>  
            ![](figures/选择部署来源-6.png "选择部署来源-6")

        2.  步骤“解压文件“与“执行shell命令“保持默认配置即可。

    5.  选择“参数设置“页签，根据容器镜像服务登录指令填写参数。

        登录指令通过控制台获取，操作方式请参考[1](步骤六-构建应用.md#li181937383409)。
...
3.  执行部署任务。
    ...
    任务执行耗时约1分钟，当出现如[图5](#fig1789294092219)所示的页面时，表示任务执行成功。

    若任务执行失败，请参照[部署-常见问题-任务步骤问题](https://support.huaweicloud.com/deployman_faq/deployman_faq_1017.html)排查。

    **图 5**  部署成功<a name="fig1789294092219"></a>  
    ![](figures/部署成功-7.png "部署成功-7")

4.  验证部署结果。
    1.  登录[云容器引擎控制台](https://console.huaweicloud.com/cce2.0/?region=&locale=zh-cn#/app/dashboard)
    2.  单击页面左侧导航“工作负载  \>  无状态负载（Deployment）“，页面中显示5条记录，状态均为“运行中“，如[图6](#fig20346104462213)所示。

        **图 6**  无状态负载（Deployment）<a name="fig20346104462213"></a>  
        ![](figures/无状态负载（Deployment）.png "无状态负载（Deployment）")

    3.  <a name="li15106105013546"></a>在列表中单击“vote“，选择“访问方式“页签，单击“添加Service“。
    4.  访问类型选择“节点访问（NodePort）“，输入Service名称“phoenix-vote“、容器端口“80“、，其余配置保持默认，单击“创建“保存，如[图7](#fig741864752213)所示。

        **图 7**  添加Service<a name="fig741864752213"></a>  
        ![](figures/添加Service.png "添加Service")

    5.  <a name="li103731454205720"></a>返回“工作负载 \> 无状态负载（Deployment）”页面，单击“vote“所对应的外部访问地址。

        页面自动跳转至如[图8](#fig17674115214229)所示的页面，在导航栏中可看到菜单项“门店网络“。

        **图 8**  用户端UI<a name="fig17674115214229"></a>  
        ![](figures/用户端UI-8.png "用户端UI-8")

    6.  返回“无状态负载（Deployment）“页面，在列表中单击“result“，参照流程[3](#li15106105013546)\~[5](#li103731454205720)配置“result“（其中），最终可跳转至如[图9](#fig1934191491310)所示页面。

        **图 9**  管理端UI<a name="fig1934191491310"></a>  
        ![](figures/管理端UI-3.png "管理端UI-3")
